...
def _run_seed_analysis(db: Session) -> None:
    ...
    image_key = f"{SEED_PROJECT_ID}/images/bolt_in_front_of_plane.png"

    already = db.query(Submission).filter(Submission.image_id == image_key).first()
    if already:
    ...
    try:
        ...
        db.commit()
        logger.info(
            "[seed] Detection complete — result: %s", result.pass_fail.upper()
        )
